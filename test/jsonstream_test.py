...
JSONStream = jsonstream_import("JSONStream")
...
def test_unexpected_comma():
    try:
        _jsonstream(',')
    except UnexpectedCharacter as error:
        assert error.args[0] == ','
    else:
        assert False


def test_unexpected_colon():
    try:
        _jsonstream(':')
    except UnexpectedCharacter as error:
        assert error.args[0] == ':'
...
def test_unexpected_open_bracket():
    try:
        _jsonstream('[1[')
    except UnexpectedCharacter as error:
        assert error.args[0] == '['
...
def test_unexpected_close_bracket():
    try:
        _jsonstream(']')
    except UnexpectedCharacter as error:
        assert error.args[0] == ']'
...
def test_unexpected_open_brace():
    try:
        _jsonstream('[1{')
    except UnexpectedCharacter as error:
        assert error.args[0] == '{'
